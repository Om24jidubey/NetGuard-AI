...
# Safely initialize Groq client without crashing the entire app if the key is missing in production
...
MODEL = "qwen/qwen3.8-27b"  # updated to available Groq model
...
"""
    ...
    if client is None:
        return f"**Warning: GROQ_API_KEY is not configured.**\n\nFallback Explanation: This was flagged as a {attack_type} attack (Severity: {severity.upper()}). Please configure the API key in the Hugging Face Spaces Secrets to enable AI analysis."
...
"""
    ...
    if client is None:
        return "I am currently running in fallback mode because no GROQ_API_KEY was provided in the Hugging Face Spaces environment secrets. Please add the API key to enable the AI assistant."
    ...
    try:
        ...
        return f"**Chat Engine Error:** {str(e)}"
...
"""
    if client is None:
        return f"**Analysis Complete:** Analyzed {total} samples. Found {num_attacks} anomalies.\n\n(AI Summarization is disabled because GROQ_API_KEY is missing)."
